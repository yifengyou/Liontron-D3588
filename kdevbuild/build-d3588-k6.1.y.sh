...
# apply patch
if ls "${WORKDIR}/kernel-6.1.y/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${WORKDIR}/kernel-6.1.y/*.patch
fi

if [ -d ${WORKDIR}/kernel-6.1.y ]; then
  ls -alh ${WORKDIR}/kernel-6.1.y/
  cp -a ${WORKDIR}/kernel-6.1.y/* .
  ls -alh
fi
...
cp -f ./arch/arm64/boot/Image /mnt/vmlinuz-6.1.y-kdev
cp -f .config /mnt/config-6.1.y-kdev
cp -f ./System.map /mnt/System.map-6.1.y-kdev
touch /mnt/initrd.img-6.1.y-kdev

cp -f ${WORKDIR}/kernel-6.1.y/extlinux.conf /mnt/
cp -f ${WORKDIR}/kernel-6.1.y/armbian_first_run.txt /mnt/
