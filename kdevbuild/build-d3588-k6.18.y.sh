...
WORKDIR=$(pwd)
export build_tag="D3588_k6.18.y_${set_release}_${set_desktop}"
...
#==========================================================================#
cd ${WORKDIR}
git clone https://github.com/ophub/linux-6.18.y.git linux-6.18.y.git
cd linux-6.18.y.git
ls -alh

# apply patch
if ls "${WORKDIR}/kernel-6.18.y/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${WORKDIR}/kernel-6.18.y/*.patch
fi

if [ -d ${WORKDIR}/kernel-6.18.y ]; then
  ls -alh ${WORKDIR}/kernel-6.18.y/
  cp -a ${WORKDIR}/kernel-6.18.y/* .
  ls -alh
fi
...
cp -f ./arch/arm64/boot/Image /mnt/vmlinuz-6.18.y-kdev
cp -f .config /mnt/config-6.18.y-kdev
cp -f ./System.map /mnt/System.map-6.18.y-kdev
touch /mnt/initrd.img-6.18.y-kdev

cp -f ${WORKDIR}/kernel-6.18.y/extlinux.conf /mnt/
cp -f ${WORKDIR}/kernel-6.18.y/armbian_first_run.txt /mnt/
